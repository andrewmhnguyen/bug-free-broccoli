...
void sim_ready() {
  ...
  if (algorithm1 == NON_PREEMPTIVE_PRIORITY){
    if (ready_list->length > 0){
      if (runt2 == false){
        runt2 = true;
        thread_t *threa = front(ready_list);
        deleteFront(ready_list);
        sim_dispatch(threa);
      }
    }
  }
  else if (algorithm1 == PREEMPTIVE_PRIORITY){
    if (ready_list->length > 0){
      if (runt2 == false){
        runt2 = true;
        thread_t *threa = front(ready_list);
        deleteFront(ready_list);
        running_thread = threa;
        sim_dispatch(threa);
      }
    }
  }
...
void sys_exec(thread_t *t) { 
   ...
   //if not, put it on the ready queue 
      ...
      if (algorithm1 == NON_PREEMPTIVE_PRIORITY){
        bool inserted = false;
        moveFront(ready_list);
        for (int i=0; i<ready_list->length; i++){
          if(ready_list->cursor->t->priority > t->priority){
            insertBefore(ready_list, t);
            printf("inserted %d\n", t->priority);
            inserted = true;
            break;
          }
        ...
        if (running_thread != NULL){
          if(running_thread->priority > t->priority){
            ...
            moveFront(ready_list);
            for (int i=0; i<ready_list->length; i++){
              if(ready_list->cursor->t->tid == running_thread->tid){
                inqueue = true;
                break;
              }
              moveNext(ready_list);
            ...
            if (inqueue == false){
              moveFront(ready_list);
              for (int i=0; i<ready_list->length; i++){
                if(ready_list->cursor->t->priority > running_thread->priority){
                  insertBefore(ready_list, running_thread);
                  printf("inserted during sys exec %d\n", running_thread->tid);
                  inserted2 = true;
                  break;
                }
                moveNext(ready_list);
              ...
              if (inserted2 == false){
                append(ready_list, running_thread);
                printf("inserted during sys exec at end %d\n", running_thread->tid);
              }
            }
            Node temp2 = ready_list->front;
            Node temp = time_list->front;
            for (int i=0; i<time_list->length; i++){
              if (temp2->t == temp->t){
                if (temp2->scp_time == -1){
                  temp2->scp_time = sim_time();
                }
                else{
                  temp2->scp2_time = sim_time();
                }
              }
              temp = temp->next;
            }
