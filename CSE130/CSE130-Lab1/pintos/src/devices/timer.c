...
timer_sleep (int64_t ticks) 
{
  ...
  int64_t new_wake = start + ticks;
  thread_current()->wake = new_wake;
...
static void 
my_function(struct thread* t, void* aux UNUSED)
{
  if(t->wake<=timer_ticks() && t->status == THREAD_BLOCKED){
    thread_unblock(t);
  }
}
